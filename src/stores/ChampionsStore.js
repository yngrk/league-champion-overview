...
export const useChampionsStore = defineStore('champions', {
  ...
  getters: {
    filteredByName(state) {
      return (str) => {
        return state.list.filter((ch) => ch.name.toLowerCase().includes(str.toLowerCase()))
      }
    },
    filteredByRole(state) {
      return (role) => {
        ...
        switch (role) {
          ...
          case 'adc':
            return state.list.filter((champion) => champion.tags.some((tag) => adc.includes(tag)))
          case 'support':
            return state.list.filter((champion) =>
    ...
    filter(state) {
      return (str, role) => {
        ...
        switch (role) {
          case 'top':
            list = list.filter((champion) => champion.tags.some((tag) => topLane.includes(tag)))
            break
          case 'mid':
            list = list.filter((champion) => champion.tags.some((tag) => midLane.includes(tag)))
            break
          case 'jungle':
            list = list.filter((champion) => champion.tags.some((tag) => jungle.includes(tag)))
            break
          case 'adc':
            list = list.filter((champion) => champion.tags.some((tag) => adc.includes(tag)))
            break
          case 'support':
            list = list.filter((champion) => champion.tags.some((tag) => support.includes(tag)))
            break
          default:
        }

        // filter by name
        list = list.filter((ch) => ch.name.toLowerCase().includes(str.toLowerCase()))
